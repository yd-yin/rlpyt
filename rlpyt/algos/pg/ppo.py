...
from rlpyt.algos.pg.base import PolicyGradientAlgo, OptInfo, ConOptInfo
from rlpyt.agents.base import AgentInputs, CONAgentInputs, AgentInputsRnn
...
class PPOCON(PolicyGradientAlgo):
    """
    Proximal Policy Optimization algorithm.  Trains the agent by taking
    multiple epochs of gradient steps on minibatches of the training data at
    each iteration, with advantages computed by generalized advantage
    ...
    opt_info_fields = tuple(f for f in ConOptInfo._fields)
    ...
    def __init__(
        ...
        if optim_kwargs is None:
            optim_kwargs = dict()
        save__init__args(locals())
        self.opt_info_fields = tuple(f for f in ConOptInfo._fields)
    ...
    def optimize_agent(self, itr, samples):
        ...
        """
        recurrent = self.agent.recurrent
        agent_inputs = CONAgentInputs(  # Move inputs to device once, index there.
            observation=samples.env.observation,
            feature=samples.agent.feature,
        )
        agent_inputs = buffer_to(agent_inputs, device=self.agent.device)
        if hasattr(self.agent, "update_obs_rms"):
            self.agent.update_obs_rms(agent_inputs.observation)
        ...
        if recurrent:
            ...
            init_rnn_state = samples.agent.agent_info.prev_rnn_state[0]  # T=0.
        T, B = samples.env.reward.shape[:2]
        opt_info = ConOptInfo(*([] for _ in range(len(ConOptInfo._fields))))
        # If recurrent, use whole trajectories, only shuffle B; else shuffle all.
        batch_size = B if self.agent.recurrent else T * B
        mb_size = batch_size // self.minibatches
        acc_size = 10
        for _ in range(self.epochs):
            for idxss in iterate_mb_idxs(batch_size, mb_size, shuffle=True):
                pi_losses = torch.zeros(mb_size // acc_size).float()
                value_losses = torch.zeros(mb_size // acc_size).float()
                entropys = torch.zeros(mb_size // acc_size).float()
                perplexitys = torch.zeros(mb_size // acc_size).float()
                self.optimizer.zero_grad()
                for i, idxs in enumerate(iterate_mb_idxs(mb_size, acc_size, shuffle=True)):
                    idxs = idxss[idxs]
                    T_idxs = slice(None) if recurrent else idxs % T
                    B_idxs = idxs if recurrent else idxs // T
                    rnn_state = init_rnn_state[B_idxs] if recurrent else None
                    # NOTE: if not recurrent, will lose leading T dim, should be OK.
                    loss, pi_losses[i], value_losses[i], entropys[i], perplexitys[i] = self.loss(
                        *loss_inputs[T_idxs, B_idxs], rnn_state)
                    loss /= (mb_size // acc_size)
                    loss.backward()
                ...
                perplexity = perplexitys.sum()
                pi_loss = pi_losses.mean()
                value_loss = value_losses.mean()
                opt_info.loss.append(loss.item())
                opt_info.pi_loss.append(pi_loss.item())
                opt_info.value_loss.append(value_loss.item())
                opt_info.gradNorm.append(grad_norm.clone().item())  # backwards compatible
                opt_info.entropy.append(entropy.item())
                opt_info.perplexity.append(perplexity.item())
                self.update_counter += 1
        if self.linear_lr_schedule:
...
class PPOACC(PolicyGradientAlgo):
    """
    Proximal Policy Optimization algorithm.  Trains the agent by taking
    multiple epochs of gradient steps on minibatches of the training data at
    each iteration, with advantages computed by generalized advantage
    ...
    def optimize_agent(self, itr, samples):
        ...
        for _ in range(self.epochs):
            self.agent.reset()
            entropys = torch.zeros(mb_size).float()
            perplexitys = torch.zeros(mb_size).float()
            for idxs in iterate_mb_idxs(batch_size, mb_size, shuffle=False):
                ...
                for i, idx in enumerate(idxs):
                    ...
                    B_idx = idx // T
                    # NOTE: if not recurrent, will lose leading T dim, should be OK.
                    loss, entropys[i], perplexitys[i] = self.loss(
                        *loss_inputs[T_idx, B_idx], B_idx)
                    loss /= mb_size
                    loss.backward()

                entropy = entropys.sum() / mb_size
                perplexity = perplexitys.sum() / mb_size
    ...
    def loss(self, agent_inputs, action, return_, advantage, valid, old_dist_info,
        ...
        for feedforward or recurrent agents.
        """
        dist_info, value = self.agent(*agent_inputs, idx)
        dist = self.agent.distribution
        ratio = dist.likelihood_ratio(action, old_dist_info=old_dist_info,
            new_dist_info=dist_info)
        surr_1 = ratio * advantage
