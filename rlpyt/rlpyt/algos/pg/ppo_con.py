        ...
        for _ in range(self.epochs):
            ...
            for idxs in iterate_mb_idxs(batch_size, mb_size, shuffle=True):
                ...
                for i, idx in enumerate(idxs):
                    T_idx = slice(None) if recurrent else idx % T
                    B_idx = idxs if recurrent else idx // T
                    rnn_state = init_rnn_state[B_idx] if recurrent else None
                    # NOTE: if not recurrent, will lose leading T dim, should be OK.
                    loss, entropys[i], perplexitys[i] = self.loss(
                        *loss_inputs[T_idx, B_idx], rnn_state)
                    loss /= mb_size
                    loss.backward()
